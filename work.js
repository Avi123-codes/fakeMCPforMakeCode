...
+'    </select>'
+'    <input id="model" placeholder="gpt-4o-mini or gemini-2.5-flash" style="flex:1;padding:8px;border-radius:8px;border:1px solid #29324e;background:#0b1020;color:#e6e8ef">'
+'  </div>'
...
const sysFor=(t)=>{
  ...
  let ns='basic,input,music,led,radio,pins,loops';
...
function askValidated(provider, apiKey, mdl, sys, user, target){
  ...
  function oneAttempt(extraSys, insist){
    ...
    logLine('Sending to '+provider+' ('+(mdl||'default')+').');
  ...
  return oneAttempt(null,false).then(res=>{
    ...
    if(!finalRes || !finalRes.code || !finalRes.code.trim()){
      ...
      return stubForTarget(target);
    }
    return finalRes.code;
  });
}
